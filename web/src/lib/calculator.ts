...
export function getAvailableContexts(maxContext: number) {
  return CONTEXT_PRESETS.filter((context) => context.tokens <= maxContext);
}

export function sortQuantizations(
  quantizations: {
    filename: string;
    bits: number;
  }[],
...
export function calculateContextSizeGb(
  numLayers,
  numKvHeads,
  headDim,
  contextLength,
...
export function calculateEstimate(input: ModelData) {
  const modelSizeGb = calculateModelSizeGb(
    input.paramsBillions,
    input.bitsPerWeight,
  );
...
export function formatQuantizationLabel(bitsPerWeight: number) {

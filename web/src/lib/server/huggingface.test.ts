...
describe('huggingface helpers', () => {
  it('normalizes full Hugging Face URLs to owner/repo format', () => {
    expect(normalizeRepoInput('https://huggingface.co/unsloth/Qwen3.5-4B-GGUF')).toBe(
  ...
  it('follows base_model metadata recursively', async () => {
    ...
    await expect(resolveBaseModel('unsloth/Qwen3.5-4B-GGUF', fetchImpl)).resolves.toEqual({
  ...
  it('builds the client payload from Hugging Face responses', async () => {
    ...
    const payload = await resolveModelPayload('unsloth/Qwen3.5-4B-GGUF', fetchImpl);
